...
Deliver a production-ready Bun/Hono backend that securely accepts WordPress content, maintains a PostgreSQL + pgvector retrieval index, runs grounded multi-turn conversations through LangGraph and the OpenAI Responses API, and returns complete answers with citations and recommendation cards.
...
- [ ] Implement `GET /health` with database and optional Redis status.
...
1. **Given** an empty supported PostgreSQL database, **when** migrations run, **then** all launch tables, constraints, extensions, and indexes are created successfully.
...
- [ ] Enable `vector` and `pgcrypto` through migrations.
- [ ] Implement configuration, API-key, data-source, content, embedding, conversation, usage, admin, and checkpoint tables.
- [ ] Add foreign keys, uniqueness constraints, checks, GIN indexes, and vector indexes.
...
2. **Given** a natural-language query, **when** semantic retrieval runs, **then** vector results retain source identity, direct URLs, and matched metadata.
3. **Given** candidates from multiple source kinds, **when** results are merged, **then** scores are normalized and duplicate content is removed.
4. **Given** review evidence, **when** it contributes to a recommendation, **then** it remains linked to its parent listing and is not returned as a listing card.
5. **Given** inactive, deleted, disallowed, or URL-less content, **when** retrieval runs, **then** that content is not returned as a recommendation candidate.

**Tasks**

- [ ] Implement listing structured and vector retrieval.
- [ ] Implement review structured and vector retrieval with parent linkage.
- [ ] Implement WordPress-content structured and vector retrieval.
- [ ] Add typed metadata, date/timezone, and optional distance filtering.
- [ ] Normalize and merge source-specific scores.
...
4. **Given** a successful or failed turn, **when** processing ends, **then** messages, citations, tool calls, response IDs, usage, latency, and status are auditable.
...
- [ ] Verify and configure the launch Responses API and embedding models.
...
1. **Given** an authorized diagnostics request, **when** it runs, **then** it reports dependency health, allowlist version, source counts, and latest indexing state.
2. **Given** an authorized usage query with a date range, **when** it runs, **then** it returns chat, indexing, latency, token, retrieval, and error aggregates without exposing private message content.
...
- [ ] Instrument chat, retrieval, indexing, database-pool, OpenAI, and error metrics.
...
1. **Given** a production-like environment, **when** deployment runs, **then** dependencies install, configuration validates, the database is backed up and migrated, the service restarts, and health checks pass.
2. **Given** a previous database backup and environment backup, **when** recovery is rehearsed, **then** the service, authentication, diagnostics, indexed content, and chat smoke test return to a usable state.
3. **Given** expected launch concurrency and indexing volume, **when** load tests run, **then** agreed latency, error, connection-pool, payload, and resource thresholds are met.
4. **Given** network, OpenAI, database, or optional Redis degradation, **when** resilience tests run, **then** failures are bounded, observable, and do not corrupt content or conversation state.
5. **Given** a security and privacy review, **when** release is approved, **then** no critical/high finding remains and retention/deletion procedures are documented.

**Tasks**

- [ ] Create production environment, reverse-proxy, HTTPS, service-manager, and deployment templates.
...
- [ ] Load-test chat, indexing batches, vector retrieval, and database pooling.
...
- [ ] Rehearse database restore, environment restore, key rotation, rollback, and reindex coordination.
