...
8. **Given** legacy authentication, site-identity, and installation-configuration data, **when** the global-configuration cutover migrations run, **then** credentials and installation-domain rows are cleared, the retrieval allowlist moves into `app_config`, and `installation_config` is removed.
